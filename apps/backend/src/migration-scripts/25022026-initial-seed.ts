...
export default async function initialSeed({ container }: ExecArgs) {
  ...
  const nestedDefs: Array<{ handle: string; name: string; parent: string }> = [
    { handle: "sweatshirts", name: "Sweatshirts", parent: "tops" },
    { handle: "long-sleeves", name: "Long Sleeves", parent: "tops" },
    { handle: "t-shirts", name: "T-Shirts", parent: "tops" },
    { handle: "bras", name: "Bras", parent: "tops" },
    { handle: "jackets", name: "Jackets", parent: "tops" },
    { handle: "hoodies", name: "Hoodies", parent: "tops" },
    { handle: "joggers", name: "Joggers", parent: "bottoms" },
    { handle: "leggings", name: "Leggings", parent: "bottoms" },
    { handle: "shorts", name: "Shorts", parent: "bottoms" },
  ];

  for (const def of nestedDefs) {
    if (!categoryHandles.includes(def.handle) && categoryMap[def.parent]) {
      nestedCategoriesToCreate.push({
        name: def.name,
        handle: def.handle,
        is_active: true,
        is_internal: false,
        parent_category_id: categoryMap[def.parent].id,
      });
    }
  }
  ...
  if (nestedCategoriesToCreate.length > 0) {
    await createProductCategoriesWorkflow(container).run({
      input: { product_categories: nestedCategoriesToCreate },
    });
  }
  ...
  const getCategoryId = (handle: string): string | null => {
    const cat = finalCategories.find((c: any) => c.handle === handle);
