...
/**
 * The currencies the index holds prices in — the store's supported currencies.
 * Each gets its own set of price fields, so filtering and sorting work per
 * currency. Adding one is a schema change: the module reindexes on the next
 * boot. Keep in sync with the storefront's `SEARCH_PRICE_CURRENCIES`.
 */
export const PRICE_CURRENCIES = ["usd", "eur", "gbp", "dkk"] as const
