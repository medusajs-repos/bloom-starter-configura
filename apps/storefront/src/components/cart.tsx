...
const CompactCartLineItem = ({ item, cart, fields }: CartLineItemProps) => {
  const hasConfiguration = item.metadata?.configuration_summary as any
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <div className="text-sm text-zinc-600">
              ...
              {hasConfiguration && (
                <span className="block text-xs text-zinc-500 mt-1">Configured</span>
              )}
            </div>
          </div>
...
const DisplayCartLineItem = ({ item, cart, className }: CartLineItemProps) => {
  const configSummary = item.metadata?.configuration_summary as Array<{
    step_name: string
    option_name: string
    price: number
  }> | undefined
      ...
      <div className="flex-1">
        ...
        {configSummary && configSummary.length > 0 && (
          <div className="mt-2 text-xs text-zinc-600">
            <p className="font-medium text-zinc-700">Configuration:</p>
            {configSummary.map((config, idx) => (
              <p key={idx}>
                {config.step_name}: {config.option_name}
              </p>
            ))}
          </div>
        )}
  ...
  return (
    <Drawer open={isOpen} onOpenChange={(open) => (open ? openCart() : closeCart())}>
      <DrawerTrigger asChild>
        <button className="relative flex items-center justify-center p-2.5 text-neutral-700 hover:text-neutral-900 transition-colors">
