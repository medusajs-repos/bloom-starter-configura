...
const ImageGalleryEnhanced = memo(function ImageGalleryEnhanced({
  ...
  return (
    <div className="space-y-3">
      {/* Main Image */}
      <div 
        className="relative aspect-[4/5] w-full overflow-hidden bg-sand-50 group cursor-zoom-in rounded-lg"
        onClick={() => openZoom(0)}
      >
        {!!images[0]?.url && (
          <>
      ...
      {images.length > 1 && (
        <div className="grid grid-cols-2 gap-3">
          {images.slice(1).map((image, index) => (
            <div 
              key={image.id}
              className="relative aspect-[4/5] overflow-hidden bg-sand-50 group cursor-zoom-in rounded-lg"
