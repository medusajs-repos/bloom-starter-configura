...
export const useCartRegionSync = (countryCode: string) => {
  ...
  useEffect(() => {
    ...
    // CRITICAL: Don't sync cart region during checkout
    // Updating the cart clears payment sessions, which breaks checkout
    // Once a cart has addresses, it's in checkout mode and should not be modified
    const isInCheckout = !!(cart.shipping_address || cart.billing_address || cart.email)
    if (isInCheckout) {
      return
    }
