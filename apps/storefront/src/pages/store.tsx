...
const Store = () => {
  ...
  const filterGroups = [
    ...
    },
    {
      ...
      label: "Material",
      options: [
        { id: "black", label: "Black Leather" },
        { id: "brown", label: "Brown Leather" },
  ...
  const filteredAndSortedItems = useMemo(() => {
    ...
    // Apply material/color filters — match against color option value or product title
    if (selectedFilters.color?.length > 0) {
      const materialKeywords: Record<string, string[]> = {
        "black": ["black"],
        "brown": ["brown"],
        "brushed-metal": ["brushed metal", "brushed"],
        "chrome": ["chrome"],
      }
      ...
      result = result.filter((item) => {
        ...
        return selectedFilters.color.some((filterColor) => {
          const keywords = materialKeywords[filterColor.toLowerCase()] || [filterColor.toLowerCase()]
